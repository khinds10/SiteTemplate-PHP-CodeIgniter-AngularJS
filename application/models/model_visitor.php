...
class Model_visitor extends CI_Model {
	...
	/**
	 * the determined IP address via sanitized HTTP_X_FORWARDED_FOR header
	 *
	 * @var string
	 */
	public $ipAddress;
	...
	public function __construct() {
		 ...
		 * 	more aggressively check against the IP address headers a 2nd time
		 */
	 ...
	 * 	via HTTP_X_FORWARDED_FOR header if possible
	 *
	...
	protected function getClientIP() {
		...
		if (strpos ( $ipaddress, ',' ) !== false) {
			...
			if (is_array ( $ipaddressInfo )) {
				$ipaddress = array_shift ( $ipaddressInfo );
			}
		}
		return $ipaddress;
	}
	 ...
	 * 	via custom set X-Bot-Recognized header
	 *
	 * @return boolean
	 */
